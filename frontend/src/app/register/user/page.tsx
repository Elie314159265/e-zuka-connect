...
export default function UserRegisterPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        let apiUrl = `${apiBaseUrl}/api/users/`;
    ...
    return (
        <AuthPageLayout>
            <div className="text-center mb-8">
                ...
                <p className="text-gray-600">お客様 新規アカウント作成</p>
            </div>
            <form onSubmit={handleSubmit}>
                <div className="mb-4">
                    <label className="block text-gray-700 text-sm font-bold mb-2">お名前</label>
                    <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="shadow-sm appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="山田 太郎" required />
                </div>
                <div className="mb-4">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                    <div>
                        <label className="block text-gray-700 text-sm font-bold mb-2">年代</label>
                        <select value={ageGroup} onChange={(e) => setAgeGroup(e.target.value)} className="shadow-sm border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    ...
                    <div>
                        ...
                        <select value={gender} onChange={(e) => setGender(e.target.value)} className="shadow-sm border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                            <option value="">選択してください</option>
            ...
            <p className="text-center text-gray-600 text-sm mt-8">
                すでにアカウントをお持ちですか？ <Link href="/login/user" className="font-bold text-blue-600 hover:text-blue-800">ログイン</Link>
            </p>
        </AuthPageLayout>
    );
};
